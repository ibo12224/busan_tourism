...
# =============================================================================
FILE_CONFIG = {
    "MAIN_DATA": "관광지_혼잡도_찐최종결과물.csv",
    "PRED_DATA": "AI_예측_결과.csv",
    "IMG_RANK_DATA": "관광지_별_유사도_순위_refined.csv", 
    "IMG_MATRIX_DATA": "부산_관광지_유사도_최종_결과_refined.csv", 
    "REVIEW_SIM_DATA": "유사도.csv",                     
    "SENTIMENT_DATA": "관광지_감상유사도_분석(최종, TF-IDF적용).csv",                   
    "FEATURE_DATA": "관광지별_키워드_유사도_순위.csv",
    "KEYWORD_NOUN": "관광지별_키워드50_추출(정제후).csv",
    "KEYWORD_ADJ": "부산_관광지별_형용사_추출결과.csv",
    "CATEGORY_INFO": "부산_관광지명.xlsx"
}
...
def generate_spot_info_ai(spot_name):
    if "OPENAI_API_KEY" not in st.secrets: return "API Key Missing"
    try:
...
def generate_strategic_analysis(source, candidates_data, anal_type="text"):
    if "OPENAI_API_KEY" not in st.secrets: return "⚠️ API Key Missing"
    candidates_text = ""
